...
const PaymentConfirmator = (props) => {
    ...
    const handleDelivery = async (e) => {
        e.preventDefault()
        try{
            await handleAddOrderItem(filteredOrder)
            props.clearOrder()
            props.onClose()
        }catch(e){
            ...
            <div className='fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 h-[40%] sm:h-[60%]  w-[60%] p-5 rounded-lg bg-[#521C0D] overflow-scroll'>
                <form onSubmit={handleDelivery} className='h-full flex flex-col relative'>
                    ...
                    <div className='mt-auto w-full border-t p-3 flex flex-col'>
                        ...
                        <button type='submit' className='text-white bg-[#3e150a] mt-3 ml-auto px-3 py-2 rounded-md cursor-pointer hover:opacity-80 transition-all ease-in-out duration-150'>Kirim Pesanan</button>
                    </div>
                </form>
            </div>
        </div>
